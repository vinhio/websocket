...
func (c *Client) readPump() {
	...
	for {
		...
		if err != nil {
			...
			log.Error("Message from websocket", message)
			break
		}
		...
		if len(msgStr) > 8 && msgStr[:8] == "/switch " {
			...
			err = msgpack.Unmarshal(message, &item)
			if err != nil {
				panic(err)
			}
			...
			// TODO Simple update channel ID to the current channel name
			//item.Channel.ID = c.hub.name
			item.Message.Content = data.ContentText{
				Text: "This is message will be generated by server",
			}

			message, err = msgpack.Marshal(item)
			if err != nil {
				panic(err)
			}
