window.onload = function () {
  ...
  function connectToChannel(channel) {
    ...
    if (!conn || conn.readyState !== WebSocket.OPEN) {
      ...
      // If connection exists and is open, send a channel switch command using the new action-based structure
      // This is an improvement over the legacy text-based command, using a structured approach
      const switchMessage = {
        metadata: {
          version: "1.0",
          timestamp: new Date().toISOString(),
      ...
      // Send as JSON
      conn.send(JSON.stringify(switchMessage));
    ...
    if (channelName) {
      ...
      if (!channelExists) {
        ...
        if (conn && conn.readyState === WebSocket.OPEN) {
          ...
          // Send as JSON
          conn.send(JSON.stringify(createChannelMessage));
  ...
  document.getElementById("listChannelsBtn").addEventListener("click", function() {
    ...
    // Send as JSON
    conn.send(JSON.stringify(listChannelsMessage));
